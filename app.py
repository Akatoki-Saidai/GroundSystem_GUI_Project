...
from flask import Flask, Response, render_template, stream_with_context, request, redirect, url_for, send_file
import glob
import serial
import os
...
def setting():
    ...
    if request.method == "GET":
        ...
        return render_template('settings.html',port_ls=port_ls,selected_port=selected_port,gpslat=gpslat,gpslon=gpslon,default_output_file=default_output_file,alert="")
        ...
        return render_template('settings.html',port_ls=port_ls,selected_port=selected_port,gpslat=request.form["gpslat"],gpslon=request.form["gpslon"],default_output_file=request.form["default_output_file"],alert='<div class="alert alert-success" role="alert"><b>保存しました</b></div>')
    ...
    def generate_data():
        if not selected_port == "":
            ...
            if os.path.isfile(f'downlinkdata/{setting_data["default_output_file"]}'):
                ...
                writer = csv.writer(file)
                writer.writerow(["lat","lng","alt","pres","rssi","mode","bt","log","wifi","gnss","timestamp","","","",""])
            ...
            while True:
                ...
                json_data = json.dumps(
                    {'time': datetime.now().strftime('%M:%S'), 'altitude': altitude, 'pressure': pressure, 'RSSI': RSSI, 'latitude':lat, 'longitude':lng, 'D_lat':D_lat, 'M_lat':M_lat, 'S_lat':S_lat, 'D_lng':D_lng, 'M_lng':M_lng, 'S_lng':S_lng, 'Mode':Mode, 'Battery':Battery, 'Log':Log, 'Wifi':Wifi, 'GNSS':GNSS, 'velocity':velocity,'console':linep,'gpslat':float(setting_data["gpslat"]),'gpslon':float(setting_data["gpslon"])})
